...
- **送出很慢通常不是你的程式碼，是 Apps Script 冷啟動**。容器閒置後被 Google 回收，
  下次請求要先喚醒。實測同一支只回一行字的 `doGet`：冷的 24.7～48.8 秒、熱的 1.7 秒。
  **診斷方法**：連打三次 `doGet` 計時，如果第一次特別久、後面變快，就是冷啟動，
  不用再去優化寫表或寄信。
  **解法**：客人一進站就在背景 `fetch(HOOK, {method:"GET", mode:"no-cors"})` 預熱，
  填表期間依互動補打（節流 60 秒避免濫發）。實測可把送出等待從 20 秒壓到約 5 秒。
  預熱請求本身可能要跑幾十秒，但那時客人還在填表，感覺不到。
  另外，`autoResizeRows()` 純粹調列高、卻要多一次試算表往返，不要放在送出路徑上；
  真要整理版面就手動跑 `formatSheet()`。
  **想再更快就得換架構**：讓 Worker 收到訂單先寫進 KV 就回應、立刻跳成功頁，
  再用 `ctx.waitUntil()` 在背景轉給 Apps Script。可壓到 1 秒內，
  但「確認成功」的責任從 Google 移到 Cloudflare，要另外補未同步訂單的對帳機制。
